...
This repo packages [Helipad](https://github.com/Podcastindex-org/helipad) for StartOS.
...
Helipad runs the `podcastindexorg/podcasting20-helipad` image pinned via `dockerTag` in `startos/manifest/index.ts`.

1. Bump `dockerTag` in `startos/manifest/index.ts` to `podcastindexorg/podcasting20-helipad:<new version>`.
